...
| `text` | 是 | `string` | 旁白文本 |
| `expected_duration` | 是 | `number` | Agent 预估时长（秒），仅作脚本规划参考；成片旁白以 TTS `actual_duration` 为准，**不会**加速或截断来凑这个值 |
| `page_target` | 是 | `string` | 该段应对应的 UI 位置（自然语言描述） |
| `scroll_action` | 是 | `string` | 兼容字段，粗略描述该段 UI 推进；正式回放以 `actions.json` 为准 |
...
`segments.json` 负责旁白分段与时间戳，回答「什么时候讲什么」。
`actions.json` 负责 UI 动作时间轴，回答「什么时候按键、点击、滚动、拖动或输入」。

Agent 可以用 `segments.json` 的 `start` / `end` 推导 `actions.json` 的 `at` 秒数，但正式录制时由 `timeline_player.py` 读取 `actions.json`，不再逐步调用 LLM `computer_use`。
...
2. **expected_duration 合理**：参考每段文字朗读约 3–4 字/秒；偏短无妨（不会被加速），但写 `actions.json` 时必须用 narrated 后的 `start`/`end`，不要用预估值
