...
| `build_avatar.py` | 可选：本地 SadTalker 分段口型 → `video/avatar.mp4` |
...
- `presenter_config.py` — 读写 `presenter.json`、半身照、耗时估算

可选真人讲解流程见 [presenter-avatar.md](presenter-avatar.md)。
...
# 可选：本片启用真人讲解时（见 presenter-avatar.md）
python3 <skill-root>/scripts/build_avatar.py \
  --output-dir ./outputs/<run-id>
